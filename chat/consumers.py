...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        try:
            ...
            # Obtenemos el usuario. Gracias a TokenAuthMiddleware en asgi.py
            self.user = self.scope['user']

            # Rechazar conexión si el usuario no está autenticado
            if not self.user.is_authenticated:
                print(f"❌ Usuario no autenticado intentó conectar a: {self.room_name}")
                await self.close()
                return
    ...
    async def disconnect(self, close_code):
        try:
            ...
            print(f"✅ Usuario {self.user.username} desconectado de la sala: {self.room_name} (code: {close_code})")
        except Exception as e:
        ...
        try:
            ...
            if source_lang != target_language:
                ...
                if 'error' not in translation_result:
                    ...
                    print(f"⚠️ Error de traducción: {translation_result['error']}")
                    translated_text = message  # Enviar mensaje sin traducir si hay error
